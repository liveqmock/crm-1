...
var MESSAGE = (function() {
    var constants = {
    		//待办事宜跳转信息列表
    		'MSG_CUST_REPEAT':{'parentFunctionId':'01002','functionId':'1106011019',url:'/custrepeat/custRepeatSearch.action','title':i18n('i18n.login.cust_repeat')},/*疑重客户*/
...
function menuActionO(id){
	//设置为订单相关待办点击来源 
	top.orderClickSource = 'click_business_backlog';
	var data = null;
	for(var i= 0;i<MessageListOrder.length;i++){
		if(MessageListOrder[i].id==id){
			data = MessageListOrder[i];
		}
	}
	var html = createMenuOrderList(id);
	document.getElementById('tododiv').innerHTML = html;
	if(html == '<img src="../images/login/todobegin.png" style="float:left;"/>'){
		document.getElementById('tododiv').style.display = 'none';
	}else{
		document.getElementById('tododiv').style.display = 'block';
	}
	//更新代办
	var textBarOrderText = null;
	if(COUNTB == 0){
		textBarOrderText = i18n('i18n.login.toDoSo')+'('+'<span style="color:red;font-weight:bold">'+COUNTB+'</span>'+')';
	}else{
		textBarOrderText = i18n('i18n.login.toDoSo')+'('+'<span style="color:red;font-weight:bold">'+COUNTB+'</span>'+')'
		+'<img clss="todoimg" src="../images/login/todo.gif" style="vertical-align: middle;margin-top:-15px"/>';
	}
	var textBar = {
			//xtype:'splitbutton',
            text:textBarOrderText,
            iconCls: 'bmenu'
    };
	tbOrder.removeAll();
	tbOrder.add(textBar);
...
function createMenuOrderList(id){
	var newMessageListOrder = new Array();
	COUNTB = 0;
	var html = '<img src="../images/login/todobegin.png" style="float:left;"/>';
	for(var i = 0;i<MessageListOrder.length;i++){
		//如果DATA不为空，则说明是点击这条代办之后，删除代办信息中这个条代办
		//如果DATA为空，则说明是定时刷新代办
		if(!Ext.isEmpty(id)&&id==MessageListOrder[i].id){
			continue;
		}
		newMessageListOrder.push(MessageListOrder[i]);
			if(Ext.isEmpty(MessageListOrder[i].taskcount)||MessageListOrder[i].taskcount==0){
				...
				html = html+'<div class="item" onclick =menuActionO("'+MessageListOrder[i].id+'")><h1>'
					+changeDataDictionaryDesc(MessageListOrder[i].tasktype)
					+'[<span class="num">'
					+MessageListOrder[i].taskcount+'</span>]</h1><img src="../images/login/todo.gif"/></div>';
    ...
    listeners:{
    	beforehide:function(){
    		Ext.Ajax.request({
				url : 'refresh.action',
					success:function(){
						refreshMainMessage(tb.items.items[0],false);
					}
			});
    	}
    },
...
function menuAction(data){
	...
	//更新代办
	var textBarText = '<span id="todoSpan">'+createMenuHtml(COUNT,null)+'</span>';
	var textBar = {
			//xtype:'splitbutton',
            text:textBarText,
            iconCls: 'bmenu',
...
function refreshMessage(){
	 ...
	 */	
	Ext.Ajax.request({
		url : 'seacherMessageCount.action',
		jsonData:{'userID':User.userId,'deptID':User.deptId},
		success:function(response){
			var result = Ext.decode(response.responseText);
			if(result.success){
				//更新代办
				COUNT = result.exceptOrderCount+result.msgInfoCount;
				oldCount = COUNT;
				var textBarText = '<span id="todoSpan">'+createMenuHtml(COUNT,null)+'</span>';
				textBar = {
						height:25,
			            ...
			            listeners:{
		            		click:refreshMainMessage
		            	}
				        
				};
				tb.removeAll();
				tb.add(textBar);
				 // menuListOrder,就是order下拉菜单的每一条
				COUNTB = result.orderCount;
				var oldCountTb = result.orderCount;
				//更新代办
				 
				var textBarOrderText = '<span id="todoSpanOrder">'+createMenuHtml(COUNTB,'ORDER')+'</span>';
				if(COUNTB != 0){
					Ext.Ajax.request({
						...
						success:function(response){
							...
							if(orderresult.success){
								...
								document.getElementById('todoSpanOrder').innerHTML = createMenuHtml(COUNTB,'ORDER');
								MessageListOrder = messageListOrder;					// 归根结底的订单待办信息list
								var html = createMenuOrderList();				// menuListOrder,就是order下拉菜单的每一条
								document.getElementById('tododiv').innerHTML = html;
								if(html == '<img src="../images/login/todobegin.png" style="float:left;"/>'){
									document.getElementById('tododiv').style.display = 'none';
								}else{
	...
	//每3分钟刷新一下代办事宜
	setTimeout("refreshMessage()",5*60*1000);
}
function createMenuHtml(count,type){
	if(type=='ORDER'){
		if(count==0){
			return i18n('i18n.login.toDoSo')+'('+'<span style="color:red;font-weight:bold">'+count+'</span>'+')';
		}else{
			return i18n('i18n.login.toDoSo')+'('+'<span style="color:red;font-weight:bold">'+count+'</span>'+')'
			+'<img clss="todoimg" src="../images/login/todo.gif" style="vertical-align: middle;margin-top:-15px"/>';
		}
		...
		if(count==0){
			return i18n('i18n.login.toDo')+'('+'<span style="color:red;font-weight:bold">'+count+'</span>'+')';
		}else{
			return i18n('i18n.login.toDo')+'('+'<span style="color:red;font-weight:bold">'+count+'</span>'+')'
			+'<img clss="todoimg" src="../images/login/todo.gif" style="vertical-align: middle;margin-top:-15px"/>';
		}
...
function refreshMainMessage(th,isClick){
	tb.items.items[0].removeListener('click',refreshMainMessage);
    th.menu.removeAll();
	Ext.Ajax.request({
		url : 'searchExceptOrderMessage.action',
		jsonData:{'userID':User.userId,'deptID':User.deptId},
		success:function(response){
			var result = Ext.decode(response.responseText);
			if(result.success){
				...
				document.getElementById('todoSpan').innerHTML = createMenuHtml(COUNT,null);
				MessageList = messageList;								// 归根结底的待办信息list
				var menuList = createMenuList();						// menuList,就是menu下拉菜单的每一条
                th.menu.add(menuList);
                if(isClick!=false){//不是click，是window隐藏时，则不显示
                	th.menu.show();
                }
    			tb.items.items[0].addListener('click',refreshMainMessage);
			}else{
				MessageUtil.showMessage(result.message);
				window.location.href="logout.action";					// 跳出登陆
			}
...
/*function refreshOrderMessage(th){
	Ext.Ajax.request({
		url : 'searchAllMessage.action',
		jsonData:{'userID':User.userId,'deptID':User.deptId},
		success:function(response){
