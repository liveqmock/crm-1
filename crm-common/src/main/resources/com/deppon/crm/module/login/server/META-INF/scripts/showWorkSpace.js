...
function toDo(){
	if(!Ext.isEmpty(top.JumpData)){
		...
		if (innerTabs) {
	        ...
	        innerTabs.remove(n);
	        var reg =/(\/[\w\-]+){3}/;
	        if(reg.test(jumpData.url)){
	        	var realUrl=window.location.origin+jumpData.url;
	        }else{
	        	var realUrl= '/'+ getAppName()+jumpData.url;
	        }
	        //判断是否已经打开该面板   
	            n = innerTabs.add({
	                ...
	                closable: true,  
	                layout: 'fit',
	                html: '<iframe id="'+id+'frame" src="'+realUrl +'" style="height:100%;width:100%;" frameborder="0"></iframe>'
	            });
...
function clickTabpanel(id,title,url,isClose){
    ...
    if (tabs) {
        ...
        var reg =/(\/[\w\-]+){3}/;
        if(reg.test(url)){
        	var realUrl=window.location.origin+url;
        }else{
        	var realUrl= '/'+ getAppName()+url;
        }
        //判断是否已经打开该面板
        if (!n) {   
            n = tabs.add({
