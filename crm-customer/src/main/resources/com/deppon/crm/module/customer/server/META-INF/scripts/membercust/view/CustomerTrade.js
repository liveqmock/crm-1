...
Ext.define('CustomerTrade', {
	...
	searchSecondTrade:function(parentId,beforeShow){
		var  me=this;
		var param = {
				'condition':{'parentId': parentId}
		};
		var sucessFn = function(json) {
			me.getForm().findField(me.second_trade_name).getStore().removeAll();
			if (!Ext.isEmpty(json.results)) {
				me.getForm().findField(me.second_trade_name).getStore().loadData(json.results);
			}
			if(beforeShow && me.second_trade_auto_expand){
				me.getForm().findField(me.second_trade_name).expand();
			}
		}
	...
	initComponent : function() {
		...
		me.items = [ {
			...
			listeners : {
				select : function(combo, records, obj) {
					me.searchSecondTrade(records[0].get('id'),true);
					me.getForm().findField(me.second_trade_name).setValue(null);
				},
			...
			listeners:{
				beforequery:function(queryEvent,eOpts){
					var trade_store = me.getForm().findField(me.trade_name).getStore();
					var second_trade_field = me.getForm().findField(me.second_trade_name);
					var second_trade_store = second_trade_field.getStore();
					var tradeValue = me.getForm().findField(me.trade_name).getValue();
					var trade_id = null;
					...
					if(Ext.isEmpty(me.getForm().findField(me.trade_name).getValue())){
		...
		if(me.viewStatus=='view'){
			me.getForm().findField(me.second_trade_name).addCls('readonly');
			me.getForm().findField(me.trade_name).addCls('readonly');
		}
	}
});
