...
Ext.define('MemberCustomerModel',{
	...
	fields:[
		 ...
		 //可使用月结额度
		 {name:'custCategory',type:'string'},
		 //财务是否冻结
		 {name:'finOver',type:'boolean'},
		 //客户性质
		 {name:'custGroup',type:'string'},
		 //是否是大客户
		 {name:'isKeyCustomer',type:'boolean'}
	   ],
...
//			});
	...
	getCustGroupStore : function(){
		if (this.custGroupStore == null) {
			this.custGroupStore = getDataDictionaryByName(DataDictionary,'CUST_TYPE');
		}
		return this.custGroupStore;
	},
	// 客户行业
...
//			});
	...
	// 客户二级行业
	getSecondTradeStore : function() {
		if (this.secondTradeStore == null) {
			this.secondTradeStore = getDataDictionaryByName(DataDictionary,'SECOND_TRADE');
//			this.tradeStore = Ext.create('TradeStore', {
//				data : DataDictionary.TRADE
//			});
		}
		return this.secondTradeStore;
	},
	// 客户属性
...
//			});
	...
	getCustPotentialStore : function() {
		if (this.custPotentialStore == null) {
			this.custPotentialStore = getDataDictionaryByName(DataDictionary,'CARGO_POTENTIAL');
...
//			});
	...
	getCustSourceStore : function() {
		if (this.potenSourceStore == null) {
			this.potenSourceStore = getDataDictionaryByName(DataDictionary,'CUST_SOURCE');
		}
		return this.potenSourceStore;
	},
	// 客户类别
	getCustCategoryStore : function() {
		if (this.custSourceStore == null) {
			this.custSourceStore = getDataDictionaryByName(DataDictionary,'CUST_CATEGORY');
		}
...
//			});
	...
	getDeptByName:function(params,successFn,failFn){
	  	DpUtil.requestJsonAjax('../organization/acquireBusinessDeptByName.action',params,successFn,failFn);
	},
