 ...
 * 定时器声明-散客系统删除定时器
 * </p>
 * @title DeleteScatterCustomerJob.java
 * @package com.deppon.crm.module.scheduler.server.job
 * @author 唐亮
 * @version 0.1 2013-2-20
 */
public class DeleteScatterCustomerJob extends GridJob{
	...
	protected void doExecute(JobExecutionContext context)
		...
		Log logger = LogFactory.getLog(DeleteScatterCustomerJob.class);
		deleteScatterCustomer(new Date());
		logger.info(new Date() + "deleteScatterCustomerJob");
	}
	/**
	 * 
	 * <p>
	 ...
	 * </p>
	 * @author bxj
	 * @version 0.2 2012-3-19
	 * void
	 */
	public void deleteScatterCustomer(Date date){
		...
		List<String> scDeptIdList = memberManager.getScatterCustDeptLoselsAndOneYear(date,Constant.CUST_GROUP_SC_CUSTOMER);
		for (String scDept : scDeptIdList) {
			List<Member> scList = memberManager.getScatterCustLoselsAndOneYear(date,scDept,Constant.CUST_GROUP_SC_CUSTOMER);
			String[] ids = new String[scList.size()];
			for (int i=0;i<scList.size();i++) {
				ids[i] = scList.get(i).getId();
				alterMemberManager.physicalDeleteMember(scList.get(i));
			}
			
			//上传一个文件
			FileInfo fileInfo = createPromptFile(scList,"本部门今年删除散客.xls","scheduler");
	...
	public FileOutputStream getExcelFile(List<? extends Member> dataList,File file) {
		...
		for (Member pc : dataList) {
			...
			row.createCell(7).setCellValue(DataDictionaryUtil.getCodeDesc(DataHeadTypeEnum.COOPERATION_LOGISTICS,pc.getMemberExtend().getRecentCoop()));
			row.createCell(8).setCellValue(DataDictionaryUtil.getCodeDesc(DataHeadTypeEnum.COOPERATION_INTENTION,pc.getMemberExtend().getCoopIntention()));
			row.createCell(9).setCellValue(DataDictionaryUtil.getCodeDesc(DataHeadTypeEnum.CARGO_POTENTIAL, pc.getMemberExtend().getVolumePotential()));
			row.createCell(10).setCellValue(pc.getMemberExtend().getCustNeed());
		}
