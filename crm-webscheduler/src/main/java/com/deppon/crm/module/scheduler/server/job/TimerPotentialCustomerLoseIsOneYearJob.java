 ...
 */
public class TimerPotentialCustomerLoseIsOneYearJob extends GridJob{
	 ...
	 * @return void
	 * @description 潜客删除
	 * @see com.deppon.crm.module.customer.server.manager.ITimerPotentialCustomer#timerOrderCargoCollectionFailList()
	 */
	private void timerPotentialCustomerLoseIsOneYear(Date date) {
		...
		// 获得要进行系统删除的列表
		List<String> pcDeptIdList = memberManager
				.getScatterCustDeptLoselsAndOneYear(date,Constant.CUST_GROUP_PC_CUSTOMER);
		// 判断列表数据
		...
		for(String pcDeptId : pcDeptIdList){
			// 获得要进行系统删除的列表
			List<Member> pcList = memberManager
					.getScatterCustLoselsAndOneYear(date,pcDeptId,Constant.CUST_GROUP_PC_CUSTOMER);
			// 遍历潜客列表
			for (Member pc : pcList) {
				// 如何存在会员Id，不能进行删除
				if (pc.getCustGroup().equals(Constant.CUST_GROUP_RC_CUSTOMER)) {
					continue;
				}
				// 不存在会员Id，进行删除
				alterMemberManager.physicalDeleteMember(pc);
			}
			//上传一个文件
			FileInfo fileInfo = createPromptFile(pcList,"本部门今年删除潜客.xls","scheduler");
	...
	protected void doExecute(JobExecutionContext context)
		...
		fileManager = getBean("fileManager", FileManager.class);
		messageManager = getBean("messageManager", MessageManager.class);
		Log logger = LogFactory.getLog(TimerPotentialCustomerLoseIsOneYearJob.class);
		timerPotentialCustomerLoseIsOneYear(new Date());
		logger.info(new Date() + " timerPotentialCustomerLoseIsOneYearJob");
	}
	/**
	 * 
	 * <p>
