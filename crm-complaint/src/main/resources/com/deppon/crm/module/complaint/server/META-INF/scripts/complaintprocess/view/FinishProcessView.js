...
Ext.define('FinishProcessWindow', {
    ...
    ,getButtons:function(){
    	var me = this;
    	return [
	    	{text:i18n('i18n.complaint.submit'),id:'finishProcessBtnSubmit',scope:me,handler:function(btn){me.finishProcessEvent(btn);}}
	    	,{text:i18n('i18n.complaintReport.btn_close'),listeners:{scope:me,click:function(){me.close();}}}
    	];
    }
    ,finishProcessEvent:function(btn){
    	...
    	function operate(finalFeedback){
			...
			var failFn = function(response){
				btn.setDisabled(false); processingMask.hide();
	...
	,listeners:{
		...
		show:function(win){
			var btn = Ext.getCmp('finishProcessBtnSubmit');
			if(btn){
