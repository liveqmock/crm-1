...
function loadProcessFn(me,is_lock){
	...
	var processComplaintWin_show = function(){
		...
		Ext.create('ProcessComplaintWindow',{
			...
			listeners:{
				show:function(){
					if(compalint.complaintsource=='NETWORK'){
						Ext.getCmp('baseFormBusinessModel').setReadOnly(false);
						Ext.getCmp('baseFormBusinessModel').removeCls('readonly');
					}else{
						Ext.getCmp('baseFormBusinessModel').setReadOnly(true);
						Ext.getCmp('baseFormBusinessModel').addCls('readonly');
					}
				}
			}
		}).show();
...
Ext.define('ComplaintBasicPanel',{
					...
					,items:[
						...
						,{
							xtype:'button',text:'外呼',handler:function(){
								...
								'complaintId='+record.data.fid+'&phone='+record.data.tel
								parent.clickTabpanel('phoneCallout','电话外拨',url1);
							}
						}
					]
				});
	...
	,loadProcessComplaintCount:function(){
		...
		DpUtil.requestJsonAjax('count_pendingComplaint.action',{
			...
			if(me.is_complaint_or_exception == false){//投诉处理员 or 异常处理员
				Ext.getCmp('waitComplaintCount_all').setValue('待办工单('+response.searchResultMap['count']+')');
			
				if(me.reportType=='COMPLAINT'){
					Ext.getCmp('waitComplaintCount_complaint_express').setValue('快递<span style="color:red;">投诉</span>数('+response.searchResultMap['count_express']+')');
					Ext.getCmp('waitComplaintCount_complaint_unExpress').setValue('零担<span style="color:red;">投诉</span>数('+response.searchResultMap['count_unExpress']+')');
				}else{
					Ext.getCmp('waitComplaintCount_exception_express').setValue('快递<span style="color:red;">异常</span>数('+response.searchResultMap['count_express']+')');
					Ext.getCmp('waitComplaintCount_exception_unExpress').setValue('零担<span style="color:red;">异常</span>数('+response.searchResultMap['count_unExpress']+')');
				}
			}
		},function(){});
	}
			...
			me.complaintGrid = Ext.create('ComplaintGrid',{
				...
				,store:me.complaintData.initProcessComplaintStore(function(store, operation, eOpts){
					...
					if(searchConditionForm!=null){
						...
						var searchParams = {
							'complaintSearchCondition.reportType':me.reportType,
							'complaintSearchCondition.businessModel':Ext.getCmp('complaintProcessBusinessType').getValue()
						};
...
Ext.define('ComplaintProcessBtnPanel',{
	...
	,initComponent:function(){
		...
		me.parent.complaintGrid.store.load({params:{'reportType':me.reportType,'firstLoad':true}});
	}
	,height:90
	,getItems:function(){
		var me = this;
		...
		return [{
				...
				,items:[{
					...
					,items:[{
						xtype:'displayfield',
						fieldLabel:'',
						value:'快递<span style="color:red;">投诉</span>数(0)',
						id:'waitComplaintCount_complaint_express',
						labelSeparator:'',
						labelWidth:60,
						width:120
					},{
						...
						margin:'0 0 0 30',
						handler:function(){
							thisPanel.loadCompListFn(this,'EXPRESS');
						}
					},{
						xtype:'displayfield',
						fieldLabel:'',
						value:'零担<span style="color:red;">投诉</span>数(0)',
						labelSeparator:'',
						labelWidth:60,
						margin:'20 0 0 0',
						width:120,
						id:'waitComplaintCount_complaint_unExpress'
					},{
						xtype:'button',
						text:'接入零担投诉',
						margin:'20 0 0 30',
						handler:function(){
							thisPanel.loadCompListFn(this,'UNEXPRESS');
						}
					}]
				},{
					xtype:'panel'
					...
					,items:[{
						xtype:'displayfield',
						fieldLabel:'',
						value:'快递<span style="color:red;">异常</span>数(0)',
						labelSeparator:'',
						labelWidth:60,
						width:120,
						id:'waitComplaintCount_exception_express'
					},{
						xtype:'button',
						text:'接入快递异常',
						margin:'0 0 0 30',
						...
						xtype:'displayfield',
						fieldLabel:'',
						value:'零担<span style="color:red;">异常</span>数(0)',
						labelWidth:60,
						margin:'20 0 0 0',
						width:120,
						labelSeparator:'',
						id:'waitComplaintCount_exception_unExpress'
					},{
						xtype:'button',
						text:'接入零担异常',
						margin:'20 0 0 30',
					...
					xtype:'button',text:'外呼',
						width:90,
						handler:function(){
						var complaintGrid = Ext.getCmp('complaintBasicPanel_id').complaintPanel.complaintGrid;
						var selection = complaintGrid.getSelectionModel().getSelection();
						if(DpUtil.isEmpty(selection) || selection.length==0){
							MessageUtil.showMessage(i18n('i18n.complaint.msg_data_select'));return;
						}
				...
				},{scope:me,xtype:'button',text:i18n('i18n.complaint.start_process'),
					margin:'0 0 0 30',
					handler:function(){
						loadProcessFn(this);
					}}
	...
	,loadCompListFn:function(reportType,businessModel){
		...
		var me = this;
		me.parent.complaintGrid.store.load({params:{'reportType':me.reportType,'businessModel':businessModel}});
		Ext.getCmp('complaintBasicPanel_id').loadProcessComplaintCount();
	}
});
...
Ext.define('ComplaintConditionForm',{
	...
	,initComponent:function(){
		...
		me.items = me.getItems();
		this.callParent(arguments);
		Ext.getCmp('complaintProcessBusinessType').getStore().add({'code':'','codeDesc':'全部'});
		Ext.getCmp('complaintProcessBusinessType').setValue('');
	}
	/*客户工单查询  事件*/
	,searchComplaintEvent:function(){
		...
		if((DpUtil.isEmpty(Ext.getCmp('fieldName').getValue()) && !DpUtil.isEmpty(fieldValue))
		  || (!DpUtil.isEmpty(Ext.getCmp('fieldName').getValue()) && DpUtil.isEmpty(fieldValue))		
			...
			MessageUtil.showMessage("查询条件不完整,请完善查询类型及查询内容！");
			return;
		}
	...
	,getItems:function(){
		...
		var items = [
			...
			,{
				xtype:'combobox',name:'businessModel',id:'complaintProcessBusinessType'
				//调用了DataDictionary.js里面的根据名称得到对应数据字典数据的方法
				,store:getDataDictionaryByName(DataDictionary,'COMPLAINT_BUSINESS_MODEL')
				,queryMode: 'local',displayField:'codeDesc',valueField:'code',editable:false,
				fieldLabel:i18n('i18n.complaint.businessType'),labelWidth:55,width:210
			},{
		...
		return [
			{
				xtype:'noborderformpanel',layout:{type:'table',columns:6},
...
Ext.define('ComplaintGrid',{
	...
	,getColumns:function(){
		var me = this;
		return [
			{header:'接入次数',width:60,dataIndex:'numberOfLock'}
			,{
...
Ext.onReady(function(){
	...
	viewport = Ext.create('Ext.container.Viewport',{
		layout:'border','autoScroll':true,items:[Ext.create('ComplaintBasicPanel',{region:'center',id:'complaintBasicPanel_id'})]
	});
