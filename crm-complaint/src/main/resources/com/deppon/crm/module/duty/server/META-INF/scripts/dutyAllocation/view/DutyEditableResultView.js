...
Ext.define('DutyEditableResultGrid',{
	...
	initComponent:function(){
		...
		me.on('itemdblclick', function(view, record) {
			...
			dutyAddResultWindowsId.setTitle(i18n('i18n.DutyEditableResultView.updateDoResult'));//修改处理结果
			...
			{//加载部门或个人
				...
				if(record.get('divideType')==='department'){
					...
					Ext.getCmp('dutyBulletinGridId').getStore().each(function(inforUser){
	...
	isExistDeptEmp:function(parm){
		var flag = false;
		var girdStore = this.getStore();
		girdStore.each(function(record){
			if(
				record.get('divideType') === parm.divideType /*划分类型匹配*/ &&
				record.get('dutyPerId') === parm.perId  /*ID相同*/
			){flag = true;return false;/*表示已存在当前部门或个人*/}
		});
		return flag;
	},
	/**
	...
	deleteBulletinByPerName:function(reusultModel){
		var bulletinStore = Ext.getCmp('dutyBulletinGridId').getStore();/*处理对象*/
...
Ext.define('DutyAddResultPanel',{
	...
	getItems:function(){
		...
		return [
		    {
		    	xtype:'combobox',name:'busAspectId',editable:false,forceSelection:true,store:me.busItemStore
		    ...
		    {
	    		xtype:'combobox',name:'busScopeId',editable:false,forceSelection:true,
			...
			{
				xtype:'combobox',name:'busTypeId',editable:false,forceSelection:true,
			...
			},
			{
				...
				xtype:'combobox',name:'reworpusType',editable:false,forceSelection:true,
			...
			},
			{
				...
				editable:false,minValue:new Date()
			},
			{
				xtype:'combobox',name:'businessLink',editable:false,forceSelection:true
                ...
                value:'RECEIVE_LINK'//‘责任家默认值收货环节’
			},
			{
				xtype:'combobox',name:'divideType',editable:false,forceSelection:true
...
Ext.define('DutyAddResultWindows',{
	extend:'PopWindow',
	title:i18n('i18n.DutyEditableResultView.addingProcessingResults'),//添加处理结果
	...
	listeners:{
		show:function(){
			...
			//加载业务项
			formPanel.loadBusItemStore(Ext.getCmp('dutyAllocationOperationWindowId').dutyDetail.reportType);
		},
		close:function(){
			var gridStore = Ext.getCmp('dutyEditableResultGridId').getStore();
		...
		return  [
			{
				...
				handler:function(){
					...
					var resultGrid = Ext.getCmp('dutyEditableResultGridId');
					...
					if(divideType === 'department'){
						...
						//验证部门不能为空 个人不需要验证
						if(Ext.isEmpty(dealName.getRawValue().trim())){
							MessageUtil.showMessage(i18n('i18n.DutyEditableResultView.dealNameIsNotEmpty'));return;
						}
					}else{
					...
					if(!(divideType===reusultModel.get('old_dealType')&&dutyPerId===reusultModel.get('old_dutyPerId'))){//若是新增old_dealType是空，肯定回去校验，如果是修改则跟原来的进行比较
						//修改过了部门或个人
						//判断当前部门或个人是否存在
						if(resultGrid.isExistDeptEmp({
							'divideType':divideType,
							'perId':dutyPerId
						})){
							MessageUtil.showMessage(i18n('i18n.DutyEditableResultView.deptUserHavePaeaseReSelect'));return;
						}
					}
                    //后进行数据加载
					{//赋值的
					...
					if(me.isUpdate && me.isUpdate==true){//修改
						if(!(divideType===reusultModel.get('old_dealType')&&dutyPerId===reusultModel.get('old_dutyPerId'))){
							resultGrid.deleteBulletinByPerName(reusultModel);
							loadEmpOrDept();
						}
					}else{//添加
						loadEmpOrDept();
