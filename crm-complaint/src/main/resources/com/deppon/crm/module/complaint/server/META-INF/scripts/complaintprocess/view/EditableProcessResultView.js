...
Ext.define('EditableProcessResultPanel',{
	...
	,addRowEvent:function(){
		...
		var m=new ProcessResultModel();
		m.set("leaveDeptName",me.up('window').complaint.leaveDeptName);
		panelStore.add(m);
...
Ext.define('EditableProcessResultGrid',{
		...
		return [
			...
			,{
				...
				,renderer:function(value,obj,record){
					me.update_bulletinByField({
						'bulletinStore':me.parent.bulletinStore,
						'jobCode':record.get('jobCode'),
						'fieldName':'feedtimelimit',
						'fieldValue':value
					});
					return value;
				}
			...
			,{
				...
				,renderer:function(value,obj,record){
					me.update_bulletinByField({
						'bulletinStore':me.parent.bulletinStore,
						'jobCode':record.get('jobCode'),
						'fieldName':'processtimelimit',
						'fieldValue':value
					});
					return value;
				}
				,width:110,dataIndex:'processtimelimit',editor:{
					xtype:'numberfield',minValue:0
				}
			},
			{
				header:i18n('i18n.complaint.process_result.dealmatters')
				,width:110,dataIndex:'dealmatters',editor:{
					xtype:'dptextfield',listeners:{
				...
				header:'处理标准'
				,width:110,dataIndex:'procStandard',
	...
	,delete_bulletinByDealName:function(opt){
		var jobCode = opt['jobCode'];//当前Result 若为 null or '' 则不删除 !Ext.isEmpty(jobCode)
		opt['bulletinStore'].each(function(bulletin_record){
			var code = bulletin_record.get('jobCode');
			if (!Ext.isEmpty(jobCode) && code == jobCode){
...
*/
	...
	/**
	 * 如果反馈时限修改，则为对应的通知对象，改变反馈时限
	 */
	,update_bulletinByField : function(opt){
		
		var jobCode = opt['jobCode'];
	 	opt['bulletinStore'].each(function(bulletin_record){
	 		var code = bulletin_record.get('jobCode');
				if (!Ext.isEmpty(jobCode) && code == jobCode){
					bulletin_record.set(opt['fieldName'],opt['fieldValue']);
				}
	 	});
	 }
		...
		Ext.create('DeptLookUpWindow',{
			...
			listeners:{
    			select:function(win,record){
    				...
    				processtimelimit = selection.get('processtimelimit');
    				feedtimelimit = selection.get('feedtimelimit');
    				me.fn_loadBulletin(record,dealType,jobCode,processtimelimit,feedtimelimit);
	...
	,employeeLookUpWindow:function(selection,bulletinStore){
		...
		var dealType = selection.get('dealType');
		var old_dealType = selection.get('old_dealType');
		var processtimelimit = selection.get('processtimelimit');
		var feedtimelimit = selection.get('feedtimelimit');
		Ext.create('EmployeeLookUpWindow',{
			title:i18n('i18n.comp.Employee.win_title'),//员工选择
			listeners:{
    			select:function(win,record){
    				if(
    					!DpUtil.isEmpty(old_dealType)
    				...
    				if(me.is_exist_noreturn_result(dealType,record.get('empId'))){
    					MessageUtil.showMessage(i18n('i18n.comp.Employee.msg.noReturn_exist'));return;
    				}
    				var old_ftaskDeptName = selection.get('old_ftaskDeptName');
    				var old_dealType = selection.get('old_dealType');
    				var old_jobCode = selection.get('jobCode');
    				...
    				if(!me.is_existBulletin(record.data,dealType,jobCode)){
    					bulletinStore.add({
							...
							,'jobCode':jobCode
							,'processtimelimit' : processtimelimit //为通知对象赋值处理时限
							,'feedtimelimit' : feedtimelimit //为通知对象赋值反馈时限
						});
    				}
	...
	,fn_loadBulletin:function(dept_record,dealType,jobCode,processtimelimit,feedtimelimit){
		var me = this;
