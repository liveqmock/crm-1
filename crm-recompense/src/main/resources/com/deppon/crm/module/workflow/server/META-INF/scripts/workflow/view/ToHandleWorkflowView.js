...
Ext.onReady(function(){
		...
		items:[{
				...
				items:[{
						...
						handler:function(){
							...
							showDetailWorkflow('doApproveWorkflow.action?processInstId='+record.get('processinstidEnc')+'&workItemId='+record.get('workItemIdEnc')+'&processDefName='+record.get('processDefName'));
						}
					}]
	 ...
	 * 底部GridPanel
	 */
	var toHandleWorkflowStore = Ext.create('ToHandleWorkflowStore');
	toHandleWorkflowStore.getProxy().extraParams={type:1};
	toHandleWorkflowStore.load();
