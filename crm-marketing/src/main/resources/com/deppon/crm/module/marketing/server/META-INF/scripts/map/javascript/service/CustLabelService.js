...
function initCustLabelStore(params,viewCustLabelSuccess,viewCustLabelFail) {
		DpUtil.requestAjax('../customer/searchLabel.action',params,viewCustLabelSuccess,viewCustLabelFail);
}
/**
 * 得到当前用户
 */
...
function addLabelToCustLabel(labelName,labelId,custData,custType,status,custLabels) {
	...
	custLabelList.add(
		...
		removeHanlder:function(){
			//删除的时候 干掉前台传给后台的参数
			 custLabels.pop(custLabelModel.data);
			 custData.data.custLabels = custLabels;

			var me= this;
			Ext.getCmp(me.up('panel').labelId).setDisabled(false);
