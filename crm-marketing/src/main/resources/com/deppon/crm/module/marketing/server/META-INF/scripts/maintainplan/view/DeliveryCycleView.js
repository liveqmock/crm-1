...
//			Ext.getCmp('deptId').setValue(json.currentDept);
//		};
//		var failureFn = function(json){
//			MessageUtil.showErrorMes(json.message);
//		};
//		var param = {};
//		CycleData.prototype.searchCurrentDept(param,successFn,failureFn); 
		this.callParent();
	},
	getItems:function(){
		var me = this;
	...
	getItems:function(){
		...
		return [{
			...
			items:[{
				...
				hidden:!isPermission('marketing.maintainplan.deliveryCycleView.btnSavePlan'),
				handler:function(){
			...
			},
			{
				...
				*20131120
				*/
				text:i18n('i18n.Cycle.delivery')+i18n('i18n.Cycle.amountDetail'),
				handler:function(){
					...
					var params = {
						...
						//发货标志
						'condition.fcAnalyseType' :CYCLE_DELIVERY
					};
					var successFn = function(json){
						var deliveryAmountDetailPopWindow=Ext.getCmp("deliveryAmountDetailPopWindowId");//获取win
						//判断此窗体在页面中是否已经存在
						if(!deliveryAmountDetailPopWindow){
							deliveryAmountDetailPopWindow=Ext.create('DeliveryAmountDetailPopWindow',{id:'deliveryAmountDetailPopWindowId'});
						}
						/**
						 * 苏玉军
						 */
						Ext.getCmp('queryDetailDate').setValue(customerInfoFormPanel.getForm().findField('queryDate').getValue());
						deliveryAmountDetailPopWindow.show();
						//获取数据
						var deliveryAmountDetailGrid = Ext.getCmp("deliveryAmountDetailGridId");
						var deliveryAmountDetailStore = deliveryAmountDetailGrid.store;
						//显示数据
						deliveryAmountDetailGrid.reconfigure(deliveryAmountDetailStore,json.columModel);
						deliveryAmountDetailGrid.scrollByDeltaX(-1);
						
						//设置每次查询时，将当前页数设置成第一页
						deliveryAmountDetailStore.loadPage(1);
				...
				load:function(store,records){
					...
					store.each(function(record){
						 if(!Ext.isEmpty(record.get("sendGoodCycle"))){
							...
							record.commit();
							var cycleListGrid=Ext.getCmp("cycleListGrid");
							if(!Ext.isEmpty(cycleListGrid.getView().getNodes()) && !Ext.isEmpty(cycleListGrid.getView().getNodes()[girdcount])){
								var cells =  cycleListGrid.getView().getNodes()[girdcount].children;
								if(!Ext.isEmpty(record.get("isMarkColor")) && record.get("isMarkColor")=="1"){//如果标识是1的话
									cells[6].style.backgroundColor='#FF9797';
								}
							}
			 			};	
  		...
  		,getColumns:function(){//查询结果列
  			return [{
 	            ...
 	            renderer : function(value){
  				...
  				header : i18n('i18n.ArrivalCycleView.inkmanId'),
  				dataIndex : 'contactId',
  				hidden:true
  			}]
  		}
...
/**
 * 描述：单击页面的本月发货金额明细按钮后弹出的窗体
 * 肖红叶
 * 2013-11-20
 */
Ext.define('DeliveryAmountDetailGrid',{
	extend:'PopupGridPanel',
	/**
	 * @auth 苏玉军
	 * @desctiption 发货明显新增日期查询条件
	 */
	tbar : [{
		id:'queryDetailDate',
			...
			listeners:{
				beforeload:function(store,operation,e){
					...
					var searchParams = {
						...
						'custGroupDayDetailCondition.fcAnalyseType' : CYCLE_DELIVERY,
						/**
						 * 苏玉军
						 */
						//发货类别
						'custGroupDayDetailCondition.sendGoodsType':Ext.getCmp('sendGoodsType').getValue()
					};
					Ext.apply(operation,{
						params : searchParams
					});
...
Ext.define('DeliveryAmountDetailPopWindow',{
	extend:'PopWindow',
	title:i18n('i18n.Cycle.delivery')+i18n('i18n.Cycle.amountDetail'),
	alias : 'widget.basicwindow',
	width:800,
	height:600,
	modal:true,
	layout:'fit',
	closeAction:'hide',
	items:[Ext.create('DeliveryAmountDetailGrid',{
		id:'deliveryAmountDetailGridId'
	})],
	buttons:[{
		text:i18n('i18n.DevelopManageView.close'),
		handler:function(){
