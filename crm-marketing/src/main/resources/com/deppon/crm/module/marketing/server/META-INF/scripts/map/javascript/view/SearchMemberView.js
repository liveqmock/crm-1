...
//		memberSearchCombox.store.removeAll();
		...
		var win=Ext.getCmp("searchMemberWinId");//获取win
		if(!win){
			win= Ext.create('SearchMemberWin',{
				'searchMemberData':searchMemberDataControl,
...
Ext.define('SearchConditionForm',{
	...
	getItems:function(){
		...
		return [
		{  
			fieldLabel:i18n('i18n.SearchMember.custNum'),
			maxLength:40,
			...
			fieldLabel:i18n('i18n.SearchMember.contactNum'),
			maxLength:40,
			maxLengthText:i18n('i18n.ManualRewardIntegralEditView.m_maxLength',40),
			name:'linkManNumber'
		},{
...
//			}
	...
	beforeLoadScatterFn:function(store, operation, eOpts){
		...
		if(searchConditionForm!=null ){
			...
			var searchParams = {
						// 会员客户编码
					'searchCustCondition.custNumber':DButil.trimString(searchCustCondition.custNumber),
						// 客户名称（企业或个人）
					'searchCustCondition.custName':DButil.trimString(searchCustCondition.custName),
						// 联系人编码
					'searchCustCondition.linkManNumber':DButil.trimString(searchCustCondition.linkManNumber),
						// 联系人姓名
					'searchCustCondition.linkManName':DButil.trimString(searchCustCondition.linkManName),
						// 手机号码(联系人手机号)
					'searchCustCondition.mobile':DButil.trimString(searchCustCondition.mobile),
						//固定电话(联系人固定电话)
					'searchCustCondition.telePhone':DButil.trimString(searchCustCondition.telePhone),
						//身份证号码
					'searchCustCondition.idCard':DButil.trimString(searchCustCondition.idCard),
					//税务登记号
					'searchCustCondition.taxregNumber':searchConditionForm.getForm().findField("taxregNumber").getValue()
			};
			Ext.apply(operation,{
				params : searchParams
				}
