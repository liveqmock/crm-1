...
//【综合信息上】（form表单对应的Model）
Ext.define('InformationTopModel',{	
	extend:'Ext.data.Model',
	fields:[
	        {name:'custNumber',  type: 'string'},			// 客户编码
	        {name:'deptName',  type: 'string'},				// 所属部门
	        {name:'degree',  type: 'string',convert: function(value){
	        	return DpUtil.changeDictionaryCodeToDescrip(value,DataDictionary.MEMBER_GRADE)}},// 客户等级
	        {name:'custName',  type: 'string'},	  			// 客户名称
	        {name:'tradeId',  type: 'string',convert: function(value){
	        	return DpUtil.changeDictionaryCodeToDescrip(value,DataDictionary.TRADE)}},// 客户行业
	        {name:'custType',  type: 'string',convert: function(value){
	        	return DpUtil.changeDictionaryCodeToDescrip(value,DataDictionary.CUSTOMER_TYPE)}},// 客户类型
	        {name:'custNature',  type: 'string',convert: function(value){
	        	return DpUtil.changeDictionaryCodeToDescrip(value,DataDictionary.CUSTOMER_NATURE)}},// 客户属性
	        {name:'taxregNumber',  type: 'string'},			// 税务登记号
	        {name:'idCard',  type: 'string'},				// 证件号码
	        {name:'billTitle',  type: 'string'},				// 发票抬头
	        {name:'custStatus',  type: 'string',convert: function(value){
	        	return  DpUtil.verifIsEffective(value)}},// 客户状态
	        {name:'procRedit',  type: 'string'}				// 临欠额度
       ]			
});
...
Ext.define('InformationTopLeftTopFormModel',{	//最内左上角form
	...
	fields:[{name:CONFIGNAME.get('Info_customerCode')},				//客户编码 
	        ...
	        {name:CONFIGNAME.get('Info_customerSecondIndustry'),convert: function(value){
	        	return DpUtil.changeDictionaryCodeToDescrip(value,DataDictionary.SECOND_TRADE);
	        }},
	        {name:CONFIGNAME.get('Info_customerPotenSouce'),convert: function(value){
...
Ext.define('InformationTopLeftbtmModel',{		//最内左上Grid
	...
	fields:[{name:CONFIGNAME.get('InfoTLB_contactPerson')},			//联系人
	        ...
	        {name:CONFIGNAME.get('InfoTLB_isMainLinkMan')}	            //是否主联系人
	]			
});
Ext.define('InformationTopRighTopModel',{		//InformationTopRighTopForm
	...
	fields:[{name:CONFIGNAME.get('InfoTLT_LastTransactionDate'),convert: function(value){
	        ...
	        {name:'latelyTradeDateExpress',convert:function(value){
	        	return new Date(value).format('yyyy-MM-dd');
	        }},{
	        	name:'durationExrpress'
	}]
});
...
Ext.define('ItemBasiInforModel',{				//基础信息ItemBasiInfor
	...
	fields:[{ name:'id' },		//id
	        ...
	        { name: CONFIGNAME.get('BI_Info_customerSecondIndustry') },	//客户二级行业
	        { name: CONFIGNAME.get('BI_Info_customerPotenSouce') },	//客户潜客来源
	        ...
	        { name:'isKeyCustomer'},//是否大客户
	        { name:'custCategory'}	//客户类别
	        ]		
});
...
Ext.define('ContractModel', {
    ...
    fields : [
		...
		{ //9.审核编号
			...
			//合同月结天数
			name:CONFIGNAME.get("contract_debtDays"),defaultValue:0,type:'number'
		}]
});
/**.
 * <p>
 ...
 * 运营分析快递MODEL<br/>
 * <p>
 * @returns {OperationAnalysisExpModel}
 */
Ext.define('OperationAnalysisExpModel', {
    extend: 'Ext.data.Model',
    fields : [
          {
      			//ID
...
//客户信息变更历史
Ext.define('CustChangeHistoryModel',{
	extend:'Ext.data.Model',
	fields:[{
		name:'id'
	},{
		name:'modifyUserName'	//变更人
	},{
		name:'department'	//变更部门
	},{
		name:'modifyDate'	//变更时间
	},{
		name:'workflowId'	//工作流号
	}]
});

//客户信息变更详细信息
Ext.define('CustChangeHistoryDetModel',{
	extend:'Ext.data.Model',
	fields:[{
		name:'fieldName'	//变更项
	},{
...
//交叉映射
Ext.define('Crm.custview.CrossMappingModel',{
	extend:'Ext.data.Model',
	fields:[{
		name:'orderSource'
	},{
...
//客户信用MODEL
Ext.define('Crm.custview.CustCreditModel',{
	extend:'Ext.data.Model',
	fields:[{
...
//流失预警model
Ext.define('Crm.custview.LoseWarningModel',{
	extend:'Ext.data.Model',
	fields:[{
		name:'warnStatus'	//预警状态
	},{
		name:'preDeliverytendTime'	//预发货开始时间
	},{
		name:'preDeliverytBiegnTime'	//预发货结束时间
	},{
		name:'isSeason'	//是否为季节性
	},{
		name:'lostCause'	//流失原因
	},{
		name:'createTime'	//预警时间
	},{
		name:'executor'	//执行人
	},{
		name:'warnStatusChangeTime'	//变更时间
	},{
		name:'lostCustRemark'	//备注
	}]
});

//运单信息
Ext.define('Crm.custview.WaybillInforGridModel',{
	extend:'Ext.data.Model',
	fields:[{
		name:'waybillNum'//单号
	},{
		name:'consignee'//收货联系人
	},{
		name:'consigneePhone'//收货人电话
	},{
		name:'consigneeMobile'//收货人手机
	},{
		name:'consigneeAddress'//收货人地址
	},{
		name:'tranProperty'//运输性质
	},{
		name:'ladingStationName'//到达网点名称（提货网点）
	},{
		name:'sendTime'//开单时间
	},{
		name:'consigneeNumber'	//收货客户名称
	}]
});

Ext.define('Crm.custview.boModel',{
	extend:'Ext.data.Model',
	fields:[{
		name:'boName'//商机名称
	},{
		...
		name:'expectDeliveryAmount'//预计发货金额
	},{
		name:'expectSuccessDate'//预计成功时间
	},{
		name:'createTime'//商机创建时间
	},{
		name:'operator'//商机执行人(负责人)
	}]
});

Ext.define('Crm.custview.mkActivityModel',{
	extend:'Ext.data.Model',
	fields:[{
		...
		name:'preferMethod'//优惠方式',
	},{
		name:'startDate'//活动开始时间',
	},{
		name:'endDate'//活动结束时间',
	},{
		name:'waybillNo'//'单号',
	},{
		name:'billingDate'//开单时间',
	}]
});