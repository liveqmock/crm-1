...
disabledDates: ["^03"]
    ...
    startDay: 0,
    /**
     * 时间设置开关，第一次为false，之后自动变为true
     * by zpj
     */
    setDefaultTime:false,
    /**
     * 默认时间值（数组），长度最大为三位，分别对应时、分、秒
     * 详细使用可参看 setTime() 方法
     * by zpj
     */
    defaultTime:[],
...
dateField.setValue('2006-05-04');
    ...
    , setTime : function(){
    	 ...
    	 */
        var me = this;
    	var t  = me.defaultTime;
    	var rs = true;
    	for (var i=0;i<t.length;i++){
    		// 检查时间值是否为数字
    		rs = rs && Ext.isNumber(t[i]);
    	}
    	if (!rs || t.length != 3){
    		// 时间检查失败，设置默认时间为当前值
    		...
    		// 时间检查成功，设置默认时间为设置值
	        me.picker.hour.setValue(t[0]);
	        me.picker.minute.setValue(t[1]);
	        me.picker.second.setValue(t[2]);
	    }
    }
    /**
