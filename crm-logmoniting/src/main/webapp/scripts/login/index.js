...
/****************************************公告栏***************************************************************/
    Ext.define('PopWinForm',{//弹窗界面的formPanel
		...
		items:[{
			...
			name:'createDate',
			labelWidth:60,
			fieldLabel:i18n('i18n.notice.activeTime')//发布时间',
		},{
			name:'modifyDate',
			labelWidth:88,
			...
			name:'visits',
			labelWidth:60,
			fieldLabel:i18n('i18n.notice.readNumber')//查阅人数
		},{
			colspan:5
		},{
			name:'content',//公告内容
	...
	Ext.define('NoticeGridPanel', {
	    ...
	    columns: [
	        {
	        	header:i18n('i18n.notice.noticeTitle'),//公告标题 , 
	        	id:'title',
	        	dataIndex: 'title',
	        	flex:1 
	        	,sortable : false
	...
	noticeGridPanel = new NoticeGridPanel();
	noticeGridPanel.on("cellclick",gripClick,this);
	
	function gripClick(e){
		var rsm = noticeGridPanel.getSelectionModel();
		var record;
		var count= rsm.getCount();
			if(count>0){//如果选择了一条记录，以弹窗显示详情，如果没有选择，则不执行
				
				record = rsm.getSelection()[0];
				
				//公告类别格式转换
				record.data.type=DpUtil.changeDictionaryCodeToDescrip(record.data.type,DataDictionary.NOTICE_TYPE);
				
				//日期格式转换
				record.data.createDate=renderDate(record.data.createDate);
				record.data.modifyDate=renderDate(record.data.modifyDate);
				var winForm = Ext.create('PopWinForm',{});
				winForm.loadRecord(record);//弹窗里面的form填充数据
				
				//公告类别格式反转
				record.data.type=changeDictionaryCodeToDescrip(record.data.type,DataDictionary.NOTICE_TYPE);
				Ext.create('Ext.window.Window',{
					title:i18n('i18n.notice.noticeDetails'),//'公告详情',
					autoScroll:true,
					height: 500,
				    width: 755,
				    layout:'fit',
		...
		items:[{
			...
			items:[{
					...
					html:'<div style="height:40px; width:100%;">'+
   		 			'<img width="43" height="30" src="../images/login/welcome.png" style="float:left"/>'+
				...
				{
					xtype:'basicpanel',
					html:'<span class="titlestyle_sec">公告栏</span>'+
...
function renderDate(value) {
	if(value != null){
		return Ext.Date.format(new Date(value), 'Y-m-d T h:m:s ');
	}else{
		return null;
	}
}
